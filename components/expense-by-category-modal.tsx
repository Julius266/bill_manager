...
interface ExpenseByCategoryModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function ExpenseByCategoryModal({ isOpen, onClose }: ExpenseByCategoryModalProps) {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const { data, error: dbError } = await supabase
        .rpc('report_expense_by_category', {
          p_year: year,
          p_month: month
        });
      ...
      const expenses: CategoryExpense[] = data;
      
      // Generar PDF
      generatePDF(expenses, year, month);
  ...
  const generatePDF = (data: CategoryExpense[], year: number, month: number) => {
    const doc = new jsPDF();
    
    // Colores de la paleta
    const primaryColor: [number, number, number] = [0, 0, 0];
    const redColor: [number, number, number] = [220, 38, 38];
    const lightRedBg: [number, number, number] = [254, 226, 226];
    ...
    doc.setFontSize(16);
    doc.text('Gastos por Categoría', 20, 32);
    ...
    // Descargar
    doc.save(`gastos-por-categoria-${year}-${month.toString().padStart(2, '0')}.pdf`);
  };
      ...
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            Selecciona el período para generar el desglose de gastos por categoría
          </DialogDescription>
        </DialogHeader>
